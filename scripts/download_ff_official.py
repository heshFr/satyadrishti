        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                if attempt < max_retries - 1:
                    wait = 2 ** attempt
                    tqdm.write(f'WARNING: download failed (attempt {attempt+1}/{max_retries}): {e}. Retrying in {wait}s...')
                    time.sleep(wait)
                else:
                    tqdm.write(f'ERROR: download failed after {max_retries} attempts: {e}. Skipping {out_file}')
    else:
